...
public class OutboxBenchmarkTests(ITestOutputHelper testOutputHelper) : BaseOutboxIntegrationTest<OutboxBenchmarkTests>(testOutputHelper)
{
    ...
    public async Task Given_EventPublisherAndConsumerUsingOutbox_When_BurstOfEventsIsSent_Then_EventsAreConsumedProperly(BusType busType, bool useOutbox, int messageCount)
    {
        ...
        OutboxSendingTask<Guid> outboxSendingTask = null;
        if (_useOutbox)
        {
            outboxSendingTask = ServiceProvider.GetRequiredService<OutboxSendingTask<Guid>>();
        ...
        if (_useOutbox)
        {
            var outputRepository = ServiceProvider.GetRequiredService<IOutboxRepository<Guid>>();
