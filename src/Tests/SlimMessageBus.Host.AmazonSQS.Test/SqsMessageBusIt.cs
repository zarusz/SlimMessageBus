...
public class SqsMessageBusIt(ITestOutputHelper output) : BaseIntegrationTest<SqsMessageBusIt>(output)
{
    ...
    public async Task Queue_Given_FifoAndBulkProduce(bool fifo, bool bulkProduce)
    {
    ...
    [InlineData(false, false)]
    [InlineData(false, true)]
    public async Task TopicPubSub_Given_FifoAndBulkProduce(bool fifo, bool bulkProduce)
    {
        var subscribers = 2;
    ...
    private string QueueName([CallerMemberName] string testName = null)
        => $"{QueueNamePrefix}_{_namePrefix}_{testName}_Queue_{_nameIndex++:00}";

    private string TopicName([CallerMemberName] string testName = null)
        => $"{QueueNamePrefix}_{_namePrefix}_{testName}_Topic_{_nameIndex++:00}";
}
