...
public class OutboxBenchmarkTestsCollection : ICollectionFixture<PostgreSqlFixture>, ICollectionFixture<RabbitMqFixture>
{
}
...
public class OutboxBenchmarkTests(ITestOutputHelper output, PostgreSqlFixture postgreSqlFixture, RabbitMqFixture rabbitMqFixture) : BaseIntegrationTest<OutboxBenchmarkTests>(output)
{
    ...
    private readonly RabbitMqFixture _rabbitMqFixture = rabbitMqFixture;
    ...
    protected override void SetupServices(ServiceCollection services, IConfigurationRoot configuration)
    {
            ...
            switch (_testParamBusType)
            {
                ...
                case BusType.RabbitMQ:
                    mbb.AddChildBus("Rabbit", mbb =>
                    {
                        ...
                        {
                            cfg.ConnectionString = _rabbitMqFixture.GetConnectionString();
        ...
        services.AddDbContext<CustomerContext>(
            options => options.UseNpgsql(_postgreSqlFixture.GetConnectionString(), x => x.MigrationsHistoryTable(HistoryRepository.DefaultTableName)));
    }
    ...
    public async Task Given_EventPublisherAndConsumerUsingOutbox_When_BurstOfEventsIsSent_Then_EventsAreConsumedProperly(BusType busType, bool useOutbox, int messageCount)
    {
        ...
        OutboxSendingTask<PostgreSqlOutboxMessage> outboxSendingTask = null;
        if (_useOutbox)
        {
            outboxSendingTask = ServiceProvider.GetRequiredService<OutboxSendingTask<PostgreSqlOutboxMessage>>();
        ...
        if (_useOutbox)
        {
            var outputRepository = ServiceProvider.GetRequiredService<IOutboxMessageRepository<PostgreSqlOutboxMessage>>();
