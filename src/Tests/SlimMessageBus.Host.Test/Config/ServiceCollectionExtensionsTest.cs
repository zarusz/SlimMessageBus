...
public class ServiceCollectionExtensionsTest
{
    ...
    public void When_AddSlimMessageBus_Given_AddServicesFromAssembly_Then_AllOfTheConsumersAreRegisteredInMSDI()
    {
        ...
        _services.AddSlimMessageBus(mbb =>
        {
            mbb.AddChildBus("bus1", mbb =>
            {
                mbb.AddServicesFromAssemblyContaining<ServiceCollectionExtensionsTest>(x => x.Namespace.Contains("Config"));
            });
        });
...
{
    public Task OnHandle(SomeMessage message) => throw new NotImplementedException();
}
