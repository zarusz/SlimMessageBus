...
public class SendInterceptorPipelineTests
{
    ...
    public async Task When_Next_Then_InterceptorIsCalledAndTargetIsCalled(bool producerInterceptorRegisterd, bool sendInterceptorRegisterd)
    {
        ...
        _busMock.BusMock
            .Setup(x => x.SendInternal<SomeResponse>(request, context.Path, request.GetType(), typeof(SomeResponse), producerSettings, context.Created, context.Expires, context.RequestId, context.Headers, _busMock.ServiceProviderMock.Object, context.CancellationToken))
            .Returns(() => Task.FromResult(response));

        var subject = new SendInterceptorPipeline<SomeResponse>(_busMock.Bus, request, producerSettings, _busMock.ServiceProviderMock.Object, context, producerInterceptors: producerInterceptors, sendInterceptors: sendInterceptors);
        ...
        _busMock.BusMock.Verify(x => x.SendInternal<SomeResponse>(request, context.Path, request.GetType(), typeof(SomeResponse), producerSettings, context.Created, context.Expires, context.RequestId, context.Headers, _busMock.ServiceProviderMock.Object, context.CancellationToken), Times.Once);
    }
}
