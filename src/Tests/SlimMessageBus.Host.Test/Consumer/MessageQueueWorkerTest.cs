...
public class MessageQueueWorkerTest
{
    ...
    public void WhenCommitThenWaitsOnAllMessagesToComplete()
    {
        ...
        _consumerInstancePoolMock.Setup(x => x.ProcessMessage(It.IsAny<SomeMessage>(), It.IsAny<IReadOnlyDictionary<string, object>>(), It.IsAny<CancellationToken>(), It.IsAny<IServiceProvider>())).Returns(() => Task.Delay(50).ContinueWith(t => { Interlocked.Increment(ref numFinishedMessages); return ((Exception)null, (AbstractConsumerSettings)null, (object)null); }, TaskScheduler.Current));
    ...
    public void GivenIfSomeMessageFailsWhenCommitThenReturnsFirstNonFailedMessage()
    {
        ...
        _consumerInstancePoolMock.Setup(x => x.ProcessMessage(It.IsAny<SomeMessage>(), It.IsAny<IReadOnlyDictionary<string, object>>(), It.IsAny<CancellationToken>(), It.IsAny<IServiceProvider>())).Returns(async () => { var e = await taskQueue.Dequeue(); return (e, null, null); });
