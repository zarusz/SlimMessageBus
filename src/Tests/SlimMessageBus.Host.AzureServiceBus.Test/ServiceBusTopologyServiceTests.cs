...
public static class ServiceBusTopologyServiceTests
{
    public class ProvisionTopologyTests
    {
        ...
        private MessageBusSettings MessageBusSettings => MessageBusBuilder.Settings;
        private MessageBusBuilder MessageBusBuilder { get; }

        [Theory]
        [InlineData(true)]
        [InlineData(false)]
        public async Task When_ProvisionTopology_Given_TopicDoesNotExistAndCanCreateTopicAndDefaultTopicProvidedOrNot_Then_DefaultTopicNotCreated(bool defaultTopicSet)
        {
            // arrange
            // Remove the default consumer (see constructor
            MessageBusBuilder.Settings.Consumers.Clear();

            var producerTopic = "test-producer-topic";
            MessageBusBuilder.Produce<SomeMessage>(x =>
            {
                if (defaultTopicSet)
                {
                    x.DefaultTopic(producerTopic);
                }
            });

            ProviderBusSettings.TopologyProvisioning.CanProducerCreateTopic = true;

            CreateTopicOptions createTopicOptions = null;

            _mockAdminClient.Setup(x => x.TopicExistsAsync(It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(ResponseTask(false));
            _mockAdminClient.Setup(x => x.CreateTopicAsync(It.IsAny<CreateTopicOptions>(), It.IsAny<CancellationToken>()))
                .Callback<CreateTopicOptions, CancellationToken>((options, _) =>
                {
                    if (options.Name == producerTopic)
                    {
            ...
            // assert
            _mockAdminClient.Verify(x => x.TopicExistsAsync(producerTopic, It.IsAny<CancellationToken>()),
                defaultTopicSet ? Times.Once : Times.Never);
            _mockAdminClient.Verify(x => x.CreateTopicAsync(It.IsAny<CreateTopicOptions>(), It.IsAny<CancellationToken>()),
                defaultTopicSet ? Times.Once : Times.Never);

            if (defaultTopicSet)
            {
                createTopicOptions.Should().NotBeNull();
                createTopicOptions.Name.Should().Be(producerTopic);
            }
            else
            {
                createTopicOptions.Should().BeNull();
            }
        }
