...
public class RuntimeTypeCache : IRuntimeTypeCache
{
    ...
    public IGenericTypeCache<Func<object, object, Func<Task>, IConsumerContext, Exception, Task<bool>>> ConsumerErrorHandlerType { get; }
    ...
    public RuntimeTypeCache()
    {
        ...
        ConsumerErrorHandlerType = new GenericTypeCache<Func<object, object, Func<Task>, IConsumerContext, Exception, Task<bool>>>(
            typeof(IConsumerErrorHandler<>),
            nameof(IConsumerErrorHandler<object>.OnHandleError),
            messageType => typeof(Task<bool>),
            messageType => [typeof(Func<Task>), typeof(IConsumerContext), typeof(Exception)]);
    }
