...
public class MessageHandler : IMessageHandler
{
        ...
        await using (var messageScope = _messageScopeFactory.CreateMessageScope(consumerInvoker.ParentSettings, message, consumerContextProperties, currentServiceProvider))
        {
            ...
            try
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (!handleErrorResult.ErrorHandled)
                    {
                        responseException = ex;
                    }
                    if (!ReferenceEquals(handleErrorResult.Response, NoResponse))
                    {
    ...
    private async Task<(bool ErrorHandled, object Response)> DoHandleError(object message, IMessageTypeConsumerInvokerSettings consumerInvoker, Type messageType, bool hasResponse, Type responseType, IMessageScope messageScope, IConsumerContext consumerContext, Exception ex)
    {
        ...
        if (consumerErrorHandler != null)
        {
            ...
            errorHandled = await consumerErrorHandlerMethod(consumerErrorHandler, message, retry, consumerContext, ex).ConfigureAwait(false);
        }

        return (errorHandled, response);
    }
